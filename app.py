...
if not st.session_state.user_auth:
    ...
    with tab_log:
        ...
        if st.button("Accéder au système", use_container_width=True):
            ...
            if user:
                ...
                st.session_state.user_email = e # Indispensable pour la DB
    ...
    with tab_reg:
        ...
        em = st.text_input("Email (identifiant)")
...
# 5. CŒUR DU LOGICIEL (ZONE CENTRALE)
...
with col_c:
    ...
    try:
        # Routage vers les fichiers views
...
# 6. SAUVEGARDE AUTOMATIQUE & NAVIGATION
st.markdown("<br>", unsafe_allow_html=True)
...
with b3:
    if st.session_state.etape_actuelle < 11:
        if st.button("SUIVANT ➡️", use_container_width=True):
            # Avant de passer à la suite, on sauvegarde l'état actuel en DB
            sauvegarder_dossier(
                st.session_state.user_email, 
                st.session_state.etape_actuelle + 1,
                st.session_state.get('faits_bruts', ''),
                st.session_state.get('branche_active', '')
            )
            st.session_state.etape_actuelle += 1
            st.rerun()
